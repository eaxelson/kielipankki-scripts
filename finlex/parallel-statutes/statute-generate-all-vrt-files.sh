...
do
	...
	if [ "$only_vrt_step" != "true" ]; then
	    ...
	    if ! ($path/statute-add-sentence-tags.pl --tag-threshold 100 --comma-threshold 150 --threshold 175 --limit 200 --filename $sentfile < $punctfile | $path/statute-add-attributes.pl > $sentfile); then
